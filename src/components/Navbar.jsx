...
const Navbar = ({ onLoginClick, user, onLogout }) => {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/" className="flex-shrink-0 cursor-pointer group flex items-center gap-2">
            <div className="bg-slate-50 p-2 rounded-lg border border-slate-200 group-hover:border-[#00E5FF] transition-colors duration-300 shadow-sm">
              <Trophy size={20} className="text-[#00E5FF] drop-shadow-sm" />
            </div>
            <h1 className="text-2xl font-extrabold italic tracking-tighter text-[#0a192f]">
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#0a192f] to-[#005f99]">CRICKET</span>
            </h1>
          </Link>
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`md:hidden bg-white/95 backdrop-blur-xl border-t border-slate-100 transition-all duration-300 ease-in-out overflow-hidden shadow-lg ${
