...
import { Globe, Smartphone, Megaphone, Scale, TrendingUp, X, Bell, Calendar } from 'lucide-react';
  ...
  const DUMMY_ANNOUNCEMENTS = [
    {
      ...
      message: 'New users can now claim a 100% deposit bonus up to ₹10,000! Check the promotions page.',
      ...
      <div className="relative pt-24 pb-10 md:pt-32 md:py-20 w-full bg-white text-slate-800 overflow-hidden flex flex-col items-center justify-center font-sans">
        ...
        <div className="relative z-10 container mx-auto px-4 text-center">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-6xl font-extrabold mb-4 md:mb-6 tracking-tight text-[#0a192f]">
            Bet Smart. <span className="text-blue-600">Win Big.</span>
          </h1>

          <p className="text-slate-600 text-base md:text-xl max-w-sm md:max-w-3xl mx-auto mb-8 md:mb-12 leading-relaxed font-medium">
            Your #1 trusted source for reliable betting site reviews, expert prediction tips, 
            real-time odds analysis, and genuine player complaint resolution.
          </p>
          ...
          <div className="inline-block w-full max-w-[340px] md:max-w-4xl lg:w-auto bg-white rounded-xl shadow-2xl shadow-slate-200/50 border border-slate-200 overflow-hidden">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:flex gap-[1px] bg-slate-200">
              ...
              <NavItem icon={Globe} label="Betting Sites" link="/sites" color="text-yellow-600" bgColor="bg-yellow-500" />
              <NavItem icon={Smartphone} label="Betting Apps" link="/apps" color="text-pink-600" bgColor="bg-pink-500" />
              <NavItem icon={TrendingUp} label="Betting Tips" link="/tips" color="text-cyan-600" bgColor="bg-cyan-500" />
              <NavItem icon={Megaphone} label="Announcements" onClick={() => setShowAnnouncements(true)} color="text-orange-600" bgColor="bg-orange-500" />
              <NavItem icon={Scale} label="Complaints" link="/complain" color="text-red-600" bgColor="bg-red-500" />
            ...
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Latest updates from the admin</p>
                </div>
              </div>
              <button 
              ...
              {loading ? (
                ...
                <div className="divide-y divide-gray-100">
                  {announcements.map((item) => (
                    <div key={item.id} className="p-5 hover:bg-blue-50/50 transition-colors group">
...
const NavItem = ({ icon: Icon, label, link, onClick, color, bgColor }) => {
  if (onClick) {
    return (
      ...
      >
        <div className={`mb-3 transition-transform duration-300 transform group-hover:-translate-y-2 group-hover:scale-110 ${color}`}>
          <Icon size={28} strokeWidth={1.5} className="md:w-8 md:h-8 drop-shadow-sm" />
        </div>
        <span className="text-xs md:text-sm font-bold text-slate-500 group-hover:text-[#0a192f] transition-colors duration-200">
          {label}
        </span>
        <div className={`absolute bottom-0 left-0 w-full h-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 scale-x-0 group-hover:scale-x-100 origin-center ${bgColor}`}></div>
      </button>
    );
  }
  return (
    <a href={link} className={`group relative flex flex-col items-center justify-center p-4 md:p-6 h-32 md:h-auto w-full lg:w-40 cursor-pointer transition-all duration-300 hover:bg-blue-50/50 bg-white`}>
      <div className={`mb-3 transition-transform duration-300 transform group-hover:-translate-y-2 group-hover:scale-110 ${color}`}>
        <Icon size={28} strokeWidth={1.5} className="md:w-8 md:h-8 drop-shadow-sm" />
      </div>
      <span className="text-xs md:text-sm font-bold text-slate-500 group-hover:text-[#0a192f] transition-colors duration-200">
