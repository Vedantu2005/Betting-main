...
// Fallback static data (used if Firestore is unavailable)
  ...
  useEffect(() => {
    try {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        // fallback to static list - no change needed
      });
      return () => unsubscribe();
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
            Top Cricket Betting <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-blue-400">Sites</span> 2025
          </h2>
          <p className="text-gray-600 max-w-2xl mx-auto">
            We've reviewed and ranked the best platforms based on odds, bonuses, and reliability.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-6">
          {sites.map((site, index) => (
            <div 
              key={site.id}
              className="group relative bg-white rounded-2xl p-6 border border-gray-200 hover:border-blue-500/50 transition-all duration-300 hover:shadow-xl transform hover:-translate-y-1"
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                ...
                {/* Column 3: Actions (Spans 3) */}
                <div className="lg:col-span-3 flex flex-col gap-3">
                  <a href={site.url || '#'} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-2 cursor-pointer px-6 py-3 bg-blue-600 text-white font-bold text-sm rounded-lg hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-600/20 transition-all duration-300">
                    VISIT SITE <ExternalLink size={16} strokeWidth={3} />
                  </a>
                  <a href={site.reviewUrl || '#'} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-2 cursor-pointer px-6 py-3 bg-white border border-gray-200 text-gray-600 font-bold text-sm rounded-lg hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 transition-colors">
