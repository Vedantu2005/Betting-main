...
import { Plus, Edit2, Trash2, Upload, X, Star, ArrowRight, ExternalLink } from 'lucide-react';
...
const CricketBookmakersManager = ({ setIsAdminLoggedIn }) => {
  ...
  useEffect(() => {
      ...
      const unsubscribe = onSnapshot(
        ...
        (error) => {
          ...
          setErrorMessage("Unable to load booksmaker from Firebase. Please refresh.");
          setIsLoading(false);
        }
      );
  ...
  const handleEdit = (bookmaker) => {
    setFormData({
      name: bookmaker.name || '',
      logo: bookmaker.logoUrl || app.logo || '',
      link: bookmaker.link || '',
      rating: bookmaker.rating || '',
    });
    setImagePreview(bookmaker.logoUrl||bookmaker.logo);
  ...
  const handleDelete =async (id) => {
    ...
    try {
          await deleteDoc(doc(db, 'bookmaker', id));
      ...
      <div className="flex-1 ml-64 p-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-3xl font-bold text-white">Cricket Bookmakers</h1>
              <p className="text-slate-400 mt-1">Manage cricket betting bookmakers</p>
            </div>
            <button
              onClick={() => {
                resetForm();
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-slate-300 font-medium text-sm mb-2">Bookmaker Name</label>
                      <input
                        type="text"
                        value={formData.name}
                    ...
                    <div>
                      <label className="block text-slate-300 font-medium text-sm mb-2">Visit Link</label>
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-slate-300 font-medium text-sm mb-2">Rating</label>
                      <div className="relative">
            ...
            {bookmakers.map((bookmaker, index) => (
              ...
              >
                ...
                <div className="absolute top-4 right-4 flex items-center gap-1 group-hover:opacity-0 transition-opacity duration-200">
                    ...
                    <span className="text-white font-bold text-sm">{bookmaker.rating}</span>
                </div>
                ...
                <div className="mt-8 mb-4 w-full h-24 flex items-center justify-center">
                  {bookmaker.logoUrl ? (
                    <img src={bookmaker.logoUrl} alt={bookmaker.name} className="max-w-[80%] max-h-full object-contain" />
                  ) : (
                    <div className="text-slate-600 font-bold text-xl">{bookmaker.name}</div>
                  )}
                </div>
